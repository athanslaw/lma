...
{	
    public function makepreference(Request $request){
		...
		//Redirect
  		return view('makepreference')->with(['stateoforigin'=>$stateoforigin, 'lga'=>$lga, 'products'=>$products, 'data'=>$datum, 'request'=>$request]);
	   }
    ...
    public function submitpreference(Request $request){
           ...
           if($request->input('state') !==null && $request->input('address') !==null && $request->input('lga') !==null && $request->input('product_type') !==null && $request->input('product_id') !==null && $request->input('lot_size') !==null){
				...
				$customer->status = 0;
           		...
           		//Redirect
           		return redirect('/preference/make')->with('success', 'Preference registered successfully');
            }
           return redirect('/preference/make')->with('data', $data);
	   }
